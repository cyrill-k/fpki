...
type LogClient interface {
	...
	LogCert(logID int64, reader io.Reader)
...
func (tc *logClient) LogCert(logID int64, reader io.Reader) {
	log.Println("Contacting Log to add cert to the tree...")
	leafValue, err := leafValueFromCertificateFile(reader)
...
func leafValueFromCertificateFile(reader io.Reader) ([]byte, error) {
	certBytes, err := common.X509CertChainBytesFromPEM(reader)
	if err != nil {
		return nil, err
	}
